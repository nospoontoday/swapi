...
function StarWarsCharacters() {
    ...
    useEffect(() => {
        const onScroll = async (event) =>  {
            ...
            const buffer = 1.5;
            //user at bottom of screen
            if (!isFetching && scrollHeight - scrollTop <= clientHeight * buffer) {
    ...
    async function getCharacters(pageParam: number) {
        const res = await fetch(`/api/swapi?page=${pageParam}`);
    ...
    return (
        <div className="mx-auto w-full max-w-5xl bg-white">
            ...
            {data &&
                ...
                <li key={i} className="border-gray-100">
                    {page.map((character) => (
